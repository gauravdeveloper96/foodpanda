...
class RestaurantController extends Controller {
    ...
    public function search(Request $request) {
        ...
        $resto_name = Restaurant::where('feature_restro', 1)->where('name', 'like', '%' . $request->restaurantName . '%')->has('Items')->select('id', 'name')->get();
...
//        dd($category->toArray());
    ...
    public function viewMenu($restro_id) {
        $items = Item::where('restaurant_id', $restro_id)->select('id', 'name')->first();
        if (isset($items)) {
            ...
            if (isset($RestroMenu)) {
//                 dd('777');
                $RestroCat = Category::whereHas('Items', function($query) use($restro_id) {
